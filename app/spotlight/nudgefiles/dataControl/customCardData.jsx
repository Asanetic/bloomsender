'use client';
//hive / data utils
import { mosyPostFormData, mosyGetData, mosyUrlParam, mosyUpdateUrlParam , deleteUrlParam, magicRandomStr, mosyGetLSData, mosyPostData  } from '../../../MosyUtils/hiveUtils';
...
export async function insertNudgeCardfiles(data) {
  //console.log(`Form nudge_files insert sent `)
 ...
 //update record 
 export async function updateNudgeCardfiles(data) {
...
function buildPayload(content, options = {}) {
  const {
    dataNode = "",
    action = "add_nudge_files"
  } = options;
  ...
  const title =
    ...
    paragraphBlock?.text?.slice(0, 50) ||
    "Untitled Document";
  ...
  return {
    ...
    nudge_files_dataNode: dataNode,
    nudge_files_mosy_action: action
  };
}
...
export async function sendNudgefilesFormAction(e, setters, content) {
  e.preventDefault();

  const form = e.target;
  const formDataObj = new FormData(form);
  const actionType = formDataObj.get('nudge_files_mosy_action');
  const dataNode = formDataObj.get('nudge_files_dataNode');
 
 //console.log(`Form nudge_files submission received action : ${actionType}`)

 const data = buildPayload(content, {
  action: actionType,
  dataNode
});
  ...
  try {
    ...
    if (actionType === 'add_nudge_files') {
      ...
      result = await insertNudgeCardfiles(data);
    }
    ...
    if (actionType === 'update_nudge_files') {
      ...
      result = await updateNudgeCardfiles(data);
    }
