...
import { AddMessagetemplates } from "../MessagetemplatesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("message_templates", node, req, newRecordId, authData);

     const insertResult = await AddMessagetemplates(newRecordId, mutatedObj, req, authData);
