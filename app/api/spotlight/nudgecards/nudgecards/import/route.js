...
import { AddNudgecards } from "../NudgecardsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("nudge_cards", node, req, newRecordId, authData);

     const insertResult = await AddNudgecards(newRecordId, mutatedObj, req, authData);
