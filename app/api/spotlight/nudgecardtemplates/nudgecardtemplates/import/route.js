...
import { AddNudgecardtemplates } from "../NudgecardtemplatesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("nudge_card_templates", node, req, newRecordId, authData);

     const insertResult = await AddNudgecardtemplates(newRecordId, mutatedObj, req, authData);
