...
import { DeleteNudgecardtemplates } from '../nudgecardtemplates/NudgecardtemplatesDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_nudge_card_templates_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'nudge_card_templates'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteNudgecardtemplates(deleteTokenDecode, whereStr);
