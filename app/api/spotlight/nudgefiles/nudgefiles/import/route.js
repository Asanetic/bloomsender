...
import { AddNudgefiles } from "../NudgefilesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("nudge_files", node, req, newRecordId, authData);

     const insertResult = await AddNudgefiles(newRecordId, mutatedObj, req, authData);
