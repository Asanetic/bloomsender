...
import { DeleteNudgefiles } from '../nudgefiles/NudgefilesDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_nudge_files_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'nudge_files'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteNudgefiles(deleteTokenDecode, whereStr);
