...
import { AddPhonebook } from "../PhonebookDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      node.client_id = newRecordId;
      
      const mutatedObj = mutateInputArray("clients", node, req, newRecordId, authData);

     const insertResult = await AddPhonebook(newRecordId, mutatedObj, req, authData);
