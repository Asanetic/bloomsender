...
import { DeletePhonebook } from '../phonebook/PhonebookDbGateway';
...
export async function GET(request) {
  ...
  if (deleteToken) {
    ...
    const table = 'clients'; // Replace with your actual table
          ...
          const fileToDelete = deleteAttachedMedia?.client_photo
         
          mosyDeleteFile(fileToDelete);
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeletePhonebook(deleteTokenDecode, whereStr);
