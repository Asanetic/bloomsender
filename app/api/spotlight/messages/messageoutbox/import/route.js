...
import { AddMessageoutbox } from "../MessageoutboxDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      node.messageid = newRecordId;
      
      const mutatedObj = mutateInputArray("messaging", node, req, newRecordId, authData);

     const insertResult = await AddMessageoutbox(newRecordId, mutatedObj, req, authData);
